...
Copia para análisis de Julio 2026.
Rutas de salida redirigidas a Analisis_Julio26/
"""
...
def find_max_jaccard(areas):
    ...
    fig.tight_layout()
    fig.savefig(f'{directory2}jaccard_featselection.png', dpi = 300)
...
def violin_s_classifications(classification_name, S, binary = False, limit = 1):
    ...
    name = f'{directory2}violin_S_{classification_name}.png'           # file name to save
    ...
    if binary: 
        ...
        name = f'{directory2}violin_S_{classification_name}_01.png' # change the file name
    ...
    for i,col in enumerate(np.flip(areas_sorted.columns.values)):
        ...
        if i != len(areas_sorted.columns.values) - 1:
            ...
            if np.shape(pca_sorted)[1] > 1:
                ...
                fig.savefig(f'{directory2}removingproteins/pc1pc2_removing{i}proteins.png', dpi = 300)
                plt.close(fig)
            ...
            fig.savefig(f'{directory2}removingproteins/pc1allsymptoms_removing{i}proteins.png', dpi = 300)
            plt.close(fig)
...
directory2 = f'{PROJECT_ROUTE}Analisis_Julio26/plots_{libreria}_{sample_type}_reduced/' if reduced else f'{PROJECT_ROUTE}Analisis_Julio26/plots_{libreria}_{sample_type}/'
...
fig.tight_layout()
fig.savefig(f'{directory2}statistics_volcano.png', dpi = 300)
...
fig.savefig(f'{directory2}explainedvariance.png', dpi = 400)
plt.close(fig)
...
ax.grid(True, alpha = 0.1)
fig.tight_layout()
fig.savefig(f'{directory2}contributionPC1.png', dpi = 300)
...
fig.savefig(f'{directory2}silhouette.png', dpi = 400)
plt.close(fig)
...
try: # PC1 - PC2
    ...
    fig.savefig(f'{directory2}scatterpc1pc2_simplified.png', dpi = 700)
    ...
    ax.set_xlabel('PC1'); ax.set_ylabel('PC2')
    fig.savefig(f'{directory2}scatterpc1pc2.png', dpi = 300)
    ...
    fig.tight_layout()
    fig.savefig(f'{directory2}scatterpc1pc2_.png', dpi = 300)
...
fig.savefig(f'{directory2}PC1_S.png', dpi = 400)


' PLOT: clinical variables heatmap sorted by S ' 
fig, ax = mapacores_sintomasS(clinical, S)
fig.savefig(f'{directory2}colormap.png', dpi = 400)
...
if not reduced: # complete and detailed figure with subplots
    ...
    fig.savefig(f'{directory2}protein_colormap_option2.png', dpi = 500)
...
fig.tight_layout()
fig.savefig(f'{directory2}average_protarea.png', dpi = 400)
...
for classification in clinical_classifications.columns.values:
    ...
    fig.savefig(f'{directory2}PC1_symptoms_{classification}.png', dpi = 300)
    ...
    fig.savefig(f'{directory2}S_symptoms_{classification}.png', dpi = 300)
...
for hue_name in ['Sex', 'Hospitalization', 'Smoking', 'Severity scale', 
    ...
    if hue_name == 'Age (years)':
        ...
        fig.savefig(f"{directory2}{hue_name.replace(' ', '')}.png")
...
if reduced == False and sample_type == 'S':
    ...
    fig.savefig(f'{directory2}proteins_individually_lateral.png', dpi = 700)
    ...
    fig.savefig(f'{directory2}proteins_individually_frente.png', dpi = 1200)
...
if reduced == False and sample_type == 'S':
    ...
    fig.savefig(f'{directory2}regression_S_individualproteins.png', dpi = 300)
    ...
    fig.tight_layout()
    fig.savefig(f'{directory2}rvalues_S_individualproteins.png', dpi = 300)
